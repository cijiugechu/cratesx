...
export const RE_VERSION =
	/^[ \t]*(?<!#)(\S+?)([ \t]*=[ \t]*)(?:({.*?version[ \t]*=[ \t]*)("|')(.*?)\4|("|')(.*?)\6)/
export const RE_FEATURES =
	/^[ \t]*(?<!#)((?:[\S]+?[ \t]*=[ \t]*.*?{.*?)?features[ \t]*=[ \t]*\[[ \t]*)(.+?)[ \t]*\]/

const RE_TABLE_HEADER = /^[ \t]*(?!#)[ \t]*\[[ \t]*(.+?)[ \t]*\][ \t]*$/
const RE_TABLE_HEADER_DEPENDENCY =
	/^(?:.+?\.)?(?:dev-)?dependencies(?:\.([^.]+?))?$/
export function findCrate(
...
export function filterCrates(items: Item[]): Item[] {
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (
			!value.key.startsWith('package.metadata') &&
			value.key.endsWith('dependencies')
		) {
...
function parseValues(data: string, parent: Item, index: number): number {
	...
	while (i++ < data.length) {
		...
		if (isWhiteSpace(ch) || isNewLine(ch) || isComma(ch)) {
			...
			isComment(ch) ||
			isGitConflictLine(current_line) ||
			isDisabledLine(current_line)
		) {
			i = skipLineData(data, i)
	...
	while (i++ < data.length) {
		...
		switch (ch) {
			...
			case "'":
				if (
					ch === opener &&
					(!multiline || data.substring(i, i + 3) === opener.repeat(3))
				) {
					if (multiline) {
						i += 2
					}
