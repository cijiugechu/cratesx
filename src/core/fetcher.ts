...
export async function fetchCrateVersions(
	...
	try {
		...
		console.error(
			`Could not check index compatibility for url "${indexServerURL}" (using sparse instead) : ${e}`,
		)
	...
	StatusBar.setText(
		'Loading',
		'👀 Fetching ' + indexServerURL.replace(/^https?:\/\//, ''),
	)

	let transformer = transformServerResponse(
		versions,
		shouldListPreRels,
		indexServerURL,
	)
		...
		return versions(item.key, indexServerURL)
			.then((crate: any) => {
				...
				crate.features?.forEach((feature: string) => {
					...
					featureCompletionItems!.set(
						feature,
						new CompletionList(
							crate.features.map((feature: string) => {
