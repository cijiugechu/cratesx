...
export class VersionCompletions implements CompletionItemProvider {
	provideCompletionItems(
		...
		if (match) {
			const crate =
				match[1] === 'version' ? findCrate(document, position.line) : match[1]
			if (!crate) return
			...
			const versionStart =
				match[1].length + match[2].length + (match[3]?.length ?? 0) + 1
...
export class FeaturesCompletions implements CompletionItemProvider {
	provideCompletionItems(
		...
		if (featuresMatch) {
			...
			const maxSatisfying =
				checkVersion(version, fetchedDep.versions)[1] ?? version
